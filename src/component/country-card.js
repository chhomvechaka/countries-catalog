...
function CountryCard({ isDarkMode, country, onCountryClick }) {
    ...
    return (
        <div className={`p-4 rounded-lg shadow-lg font-sans flex flex-col ${isDarkMode ? 'bg-gray-700 text-gray-200' : 'bg-gray-50 text-gray-800'}`}>
            <div>
                ...
                <hr className="my-2 border-t border-gray-100"/>
                <p className="text-sm mb-2"><span className="font-semibold">Native Name:</span> {nativeName || "N/A"}</p>
                <hr className="my-2 border-t border-gray-100"/>
                <p className="text-sm mb-2 truncate"><span className="font-semibold">Alternative Names:</span> {altSpellings?.join(", ") || "N/A"}</p>
                <hr className="my-2 border-t border-gray-100"/>
                <p className="text-sm mb-2 truncate"><span className="font-semibold">Calling Codes:</span> {callingCodes || "N/A"}</p>
            </div>
            <button className={`mt-auto px-4 py-2 rounded-lg ${isDarkMode ? 'bg-orange-600 text-white' : 'bg-orange-600 text-white'} hover:bg-opacity-85 transition ease-in-out duration-150`} onClick={() => onCountryClick(country)}>
                View Detail
            </button>
        </div>
    );
